...
private:
    ...
    void i_balance(boost::json::value json) {
        try {
            ...
            if (!op_balance.has_value()) create_error_response(R"({"message":"Balance not found, address: )" + name + "\"}");
            else create_success_response(R"({"message":"Ok","balance":)" + op_balance.value() + "}");
        } catch (const boost::wrapexcept<std::out_of_range> &o) {
            create_error_response(R"({"message":"Invalid data"})");
        }
    }
    ...
    bool isEnoughTokenBalance(const boost::json::value& balance, const std::string& token_name, double value) {
        ...
        for(boost::json::array::iterator it = balance_json.at("tokens_balance").as_array().begin(); it != balance_json.at("tokens_balance").as_array().end(); ++it) {
            if(it->as_object().contains(token_name)) {
                if(boost::json::value_to<double>(it->at(token_name)) < value) return false;
                return true;
            }
        }
        return false;
    ...
    void i_block_height() {
        ...
        if (!block_height.has_value()) create_error_response();
        else create_success_response(R"({"message":"Ok","block_height":)" + block_height.value() + "}");
    }
    void i_pool_size() {
    ...
    void i_tx(boost::json::value json) {
        try {
            ...
            if (!op_tx.has_value()) create_error_response(R"({"message":"Transaction not found"})");
            else create_success_response(R"({"message":"Ok","transaction":)" + op_tx.value() + "}");
        } catch (const boost::wrapexcept<std::out_of_range> &o) {
            create_error_response(R"({"message":"Invalid data"})");
        }
    }

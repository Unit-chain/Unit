...
public:
    ...
    RpcFilterBuilder(const vector<std::shared_ptr<RpcFilterObject>> &filters, const shared_ptr<boost::json::value> &parameter);
...
private:
    std::vector<std::shared_ptr<RpcFilterObject>> filters;
...
RpcFilterBuilder::RpcFilterBuilder(const vector<std::shared_ptr<RpcFilterObject>> &filters,
